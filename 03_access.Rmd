...
- eBird Basic Dataset: https://github.com/mstrimas/ebp-workshop/raw/master/data/ebd_relApr-2019_workshop.tar
- Sampling Event Data: https://github.com/mstrimas/ebp-workshop/raw/master/data/ebd_sampling_relApr-2019_workshop.tar
...
Let's take a look at this dataset. If we were working with the full EBD, we don't have enough memory to read in the whole file, but we can always read a small subset. Open a new R script (`03_access.R`) and read in the first few lines:

```{r}
library(readr)

ebd_top <- read_tsv("~/data/ebird/ebd_relApr-2019_workshop.txt", n_max = 5)
```
...
<div class="checkpoint">
  <h2>Checkpoint</h2>
  <p>Have you set the EBD path in `auk`?</p>
</div>