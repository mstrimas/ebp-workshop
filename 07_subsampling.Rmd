...
Fortunately, all three of these challenges can largely be addressed by spatiotemporal subsampling of the eBird data prior to modeling. In particular, this consists of dividing space and time up into a regular grid (e.g. 5 km x 5 km x 1 month), and only selecting a subset of checklists from each grid cell. To deal with class imbalance, we can subsample detections and non-detections separately to ensure we don't lose too many detections.
...
<div class="exercise">
  <h2>Exercise</h2>
  How has the spatiotemporal subsampling affected the overall sample size as well as the prevalence of detections?
