...
ebd_dir <- here("raw-data", "ebp-workshop-data", "ebd")
data_dir <- here("raw-data", "ebp-workshop-data", "data")
...
# part II files
book_files <- c("ebd_woothr_june_bcr27.txt", 
...
# zip
f_zip <- here("raw-data", "ebp-workshop-data.zip")
unlink(f_zip)
str_glue("zip -r {f_zip} {here('raw-data', 'ebp-workshop-data')}") %>% 
  system()
unlink(here("raw-data", "ebp-workshop-data"), recursive = TRUE)
